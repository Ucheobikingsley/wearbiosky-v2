  ...
  try {
    ...
    if (query) {
      response = await fetch(
        `https://onew.online/biosky/v1/product?query=${query.toLowerCase()}`
      );
    } else {
      response = await fetch("https://onew.online/biosky/v1/product");
    }
...
app.get("/product-details", async (req, res) => {
  ...
  try {
    response = await fetch(
      `https://onew.online/biosky/v1/product/${productId}`
    );
